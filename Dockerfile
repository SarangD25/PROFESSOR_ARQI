...
FROM node:22.22.2-alpine3.23 AS node
...
# Copy source files, SDK, and libs needed for SSR page rendering
COPY --from=server-builder /app/src ./src
COPY --from=server-builder /app/.wasp/out/sdk .wasp/out/sdk
COPY --from=server-builder /app/.wasp/out/libs .wasp/out/libs
