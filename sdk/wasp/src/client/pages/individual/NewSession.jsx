    ...
    <div className="animate-in">
      ...
      <div className="px" style={{ maxWidth: '640px' }}>
        ...
        <form onSubmit={handleStart}>
          <div className="card card-glow" style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '24px' }}>
              {Object.keys(examTopics).map(exam => (
                <button key={exam} type="button" onClick={() => setForm({...form, examName: exam, topic: ''})}
                  className={`toggle-opt${form.examName === exam ? ' selected' : ''}`}>{exam}</button>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '24px' }}>
              {(examTopics[form.examName] || []).map(topic => (
                <button key={topic} type="button" onClick={() => setForm({...form, topic})}
                  className={`toggle-opt${form.topic === topic ? ' selected' : ''}`} style={{ textAlign: 'left' }}>{topic}</button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {['easy', 'medium', 'hard'].map(d => (
                <button key={d} type="button" onClick={() => setForm({...form, difficulty: d})}
                  ...
                  style={{ flex: 1, textAlign: 'center' }}>{d.charAt(0).toUpperCase() + d.slice(1)}</button>
              ))}
            </div>
          </div>
          ...
          <button type="submit" className="btn btn-primary" disabled={loading || !form.topic}
            style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: '15px', opacity: (loading || !form.topic) ? 0.5 : 1 }}>
