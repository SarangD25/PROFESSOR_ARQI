...
export function OrgBulkQR() {
  ...
  const handleGenerate = async () => {
    ...
    try { const data = await generateQRs({ campaignId: selectedCampaign, orgId: org.id, adaptive }); setResults(data); }
  ...
  return (
    <div className="animate-in">
      ...
      <div className="px">
        ...
        {results.length === 0 ? (
          <div className="card card-glow" style={{ maxWidth: '640px' }}>
            ...
            <button onClick={handleGenerate} disabled={!selectedCampaign || generating || !org?._count?.students}
              className="btn btn-primary" style={{ opacity: (!selectedCampaign || generating) ? 0.5 : 1 }}>
            ...
            <div className="grid-4">
              {results.map((r, i) => (
                <div key={i} className="qr-card">
                  ...
                  <a href={r.paperUrl} target="_blank" rel="noreferrer"
                    style={{ fontSize: '11px', color: '#3b82f6', wordBreak: 'break-all', display: 'inline-block', marginTop: '4px' }}>Open Link</a>
                </div>
              ))}
            </div>
          </>
