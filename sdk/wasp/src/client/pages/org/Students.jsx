...
export function OrgStudents() {
  ...
  if (!org) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', color: 'var(--text3)' }}>Loading...</div>;
      ...
      <div className="px">
        ...
        <div className="card card-glow" style={{ marginBottom: '20px' }}>
          ...
          <form onSubmit={handleAdd}>
            <textarea className="form-textarea" value={csv} onChange={e => setCsv(e.target.value)}
              placeholder={"Aarav Singh, 001, 10A\nPriya Kapoor, 002, 10A\nRohan Mehta, 003, 10B"}
              style={{ minHeight: '80px', marginBottom: '12px' }} required />
                    ...
                    <tr key={s.id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '12px' }}>
                          ...
                          {(s._count?.qrPapers || 0) > 0 ? (
                            <Link to={'/org/student/' + s.id + '/report'} style={{ color: 'var(--green)', fontSize: '13px', textDecoration: 'none' }}>Report</Link>
                          ) : (
                            <span style={{ fontSize: '13px', color: 'var(--text3)', cursor: 'not-allowed' }}>Report</span>
                          )}
                        </div>
                      </td>
                    </tr>
