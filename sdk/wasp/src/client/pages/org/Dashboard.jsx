...
export function OrgDashboard() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try { await createOrg({ name: orgName }); refetch(); }
    catch (err) { setError(err.message); }
  };

  if (isLoading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', color: 'var(--text3)' }}>Loading...</div>;

  if (!org) return (
    <div style={{ maxWidth: '480px', margin: '80px auto', padding: '0 24px' }} className="animate-in">
              ...
              return (
                <div key={c.id} className="campaign-card">
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={async () => { if (confirm('Delete?')) await delCampaign({ campaignId: c.id }); }} className="btn btn-danger btn-sm">Delete</button>
                  </div>
                </div>
              );
            })}
