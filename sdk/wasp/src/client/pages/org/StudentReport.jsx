...
export function OrgStudentReport() {
  ...
  if (isLoading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', color: 'var(--text3)' }}>Loading report...</div>;
  if (!report) return <div style={{ padding: '32px', color: 'var(--red)' }}>Report not found</div>;
  ...
  if (timelineData.length >= 2) {
    ...
    if (diff > 5) { trendText = `Improving (+${diff}%)`; trendColor = 'var(--green)'; }
    else if (diff < -5) { trendText = `Declining (${diff}%)`; trendColor = 'var(--red)'; }
    else { trendText = 'Stable'; trendColor = 'var(--blue)'; }
  }
      ...
      <div className="px" style={{ maxWidth: '900px' }}>
        ...
        <div className="grid-2" style={{ marginBottom: '20px' }}>
          {Object.entries(breakdown).map(([key, data]) => (
            <div key={key} className="card-static">
        ...
        <div className="grid-2" style={{ marginBottom: '20px' }}>
          <div className="tier-card tier-excellent" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '16px' }}>
            ...
            {strongAreas.length > 0 ? strongAreas.map(([key, d]) => (
              <div key={key} style={{ fontSize: '13px', color: 'var(--green)', padding: '2px 0' }}>{typeLabels[key]} — {d.percent.toFixed(0)}%</div>
            )) : <p style={{ fontSize: '13px', color: 'var(--text3)' }}>No strong areas yet</p>}
          </div>
          <div className="tier-card tier-low" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '16px' }}>
            ...
            {weakAreas.length > 0 ? weakAreas.map(([key, d]) => (
              <div key={key} style={{ fontSize: '13px', color: 'var(--red)', padding: '2px 0' }}>{typeLabels[key]} — {d.percent.toFixed(0)}%</div>
            )) : <p style={{ fontSize: '13px', color: 'var(--text3)' }}>All areas performing well!</p>}
          </div>
        </div>
        ...
        {!assessments?.length ? (
          <div className="card-static" style={{ padding: '20px', marginBottom: '20px' }}><p style={{ color: 'var(--text3)', fontSize: '13px' }}>No manual assessments yet.</p></div>
        ) : (
          <div className="card card-glow" style={{ padding: 0, overflow: 'hidden', marginBottom: '20px' }}>
...
function ClassTestHistory({ classTests }) {
  ...
  return (
    <>
      ...
      {!classTests?.length ? (
        <div className="card-static" style={{ padding: '20px' }}><p style={{ color: 'var(--text3)', fontSize: '13px' }}>No class tests yet.</p></div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {classTests.map(paper => {
            const qs = paper.questionSets?.[0]; const attempt = qs?.attempts?.[0]; const isOpen = expanded === paper.id;
            const questions = qs ? (() => { try { return JSON.parse(qs.questions); } catch { return []; } })() : [];
            const answers = attempt ? (() => { try { return JSON.parse(attempt.answers); } catch { return []; } })() : [];
                ...
                {isOpen && attempt && (
                  <div style={{ borderTop: '1px solid var(--border)', padding: '16px 20px' }}>
                    <div className="grid-2" style={{ marginBottom: '12px' }}>
                      {weakTopics.length > 0 && (
                        <div className="tier-card tier-low" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '12px' }}>
                          <h4 style={{ fontWeight: 700, color: 'var(--red)', fontSize: '12px', marginBottom: '6px' }}>Weak Topics</h4>
                          {weakTopics.map(([concept, s], i) => (
                      ...
                      {strongTopics.length > 0 && (
                        <div className="tier-card tier-excellent" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '12px' }}>
                          <h4 style={{ fontWeight: 700, color: 'var(--green)', fontSize: '12px', marginBottom: '6px' }}>Strong Topics</h4>
                          {strongTopics.map(([concept, s], i) => (
                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', padding: '2px 0' }}>
                    ...
                    {questions.map((q, idx) => {
                      const userAns = answers[idx]; const isCorrect = checkAns(userAns, q.correctAnswer, q.type);
                      return (
                        <div key={idx} className="card-static" style={{ marginBottom: '6px', borderLeft: `3px solid ${isCorrect ? 'var(--green)' : 'var(--red)'}`, padding: '10px 12px' }}>
                          ...
                          {q.type === 'NAT' ? (
                            ...
                            <div style={{ marginTop: '4px' }}>
                              {q.options?.map((opt, i) => {
                                ...
                                return (
                                  <div key={i} style={{ fontSize: '11px', padding: '3px 0 3px 14px', color: correct ? 'var(--green)' : picked ? 'var(--red)' : 'var(--text3)' }}>
...
function checkAns(user, correct, type) {
  ...
  if (type === 'NAT') { const u = parseFloat(user), c = parseFloat(correct); return !isNaN(u) && !isNaN(c) && Math.abs(u-c) < 0.01; }
