...
function generatePDFFromPaper(paper) {
  ...
  questions.forEach((q,i) => { html += `<div class="q-block"><div class="q-title">Q${i+1}. ${q.text}</div>`;
  ...
  return (
    <div className="animate-in">
      ...
      <div className="px">
        ...
        {!papers?.length ? (
          <div className="card-static" style={{ padding: '40px', textAlign: 'center' }}><p style={{ color: 'var(--text3)' }}>No papers for this student yet.</p></div>
        ) : (
                  ...
                  <div style={{ padding: '14px 20px', cursor: 'pointer' }} onClick={() => setExpanded(isExpanded ? null : paper.id)}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {attempt ? (
                          <div style={{ textAlign: 'right' }}><div className="stat-num" style={{ fontSize: '20px' }}>{attempt.score}/{attempt.totalMarks}</div><div style={{ fontSize: '11px', color: 'var(--text3)' }}>Score</div></div>
