      ...
      <div className="max-w-5xl mx-auto p-6">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
            <div className="bg-gray-800 p-6 rounded-lg text-center max-w-sm w-full">
              ...
              <div className="flex gap-3 justify-center">
                <button
                  onClick={() => navigate(`/paper/${qrData.token}`)}
        ...
        <div className="mb-8">
          ...
          {loadingAreas ? (
            ...
            <p className="text-gray-400 bg-gray-800 p-4 rounded">No weak areas tracked yet. Complete a test to see results.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {weakAreas?.map(w => (
                <div key={w.concept} className="bg-gray-800 p-4 rounded flex justify-between items-center">
                  <span>{w.concept}</span>
        ...
        <div>
          ...
          {loadingCampaigns ? (
            ...
            <p className="text-gray-400 bg-gray-800 p-4 rounded">No campaigns available yet.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {campaigns?.map(c => (
                <div key={c.id} className="bg-gray-800 p-4 rounded border border-gray-700">
                  ...
                  <button
                    onClick={() => handleGenerateQR(c.id, c.title)}
                    disabled={generating === c.id}
                    className="mt-3 w-full bg-blue-600 hover:bg-blue-700 py-2 rounded text-sm font-medium disabled:opacity-50"
                  >
