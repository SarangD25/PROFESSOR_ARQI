...
export function OrgSegregation() {
    ...
    if (isLoading)
        return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', color: 'var(--text3)' }}>Computing student scores...</div>;
    const grouped = {};
    ...
    return (<div className="animate-in">
      ...
      <div className="px">
        ...
        <div className="grid-4" style={{ marginBottom: '32px' }}>
          {tierOrder.map(tier => (<div key={tier} className="stat-card">
        ...
        {/* Tier Sections */}
        {tierOrder.map(tier => (grouped[tier].length > 0 && (<div key={tier} style={{ marginBottom: '28px' }}>
              <h2 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '12px', color: tierColors[tier].dot }}>
                {tier} ({grouped[tier].length})
              </h2>
              <div className="grid-3">
                {grouped[tier].map(s => (<Link key={s.id} to={'/org/student/' + s.id + '/report'} className="card card-glow" style={{ textDecoration: 'none', color: 'var(--text1)' }}>
...
//# sourceMappingURL=Segregation.jsx.map