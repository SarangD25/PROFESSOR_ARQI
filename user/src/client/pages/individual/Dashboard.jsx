...
export function IndividualDashboard() {
        ...
        papers.forEach(p => {
            ...
            questions.forEach((q, idx) => {
                ...
                if (q.type === 'MCQ' && userAns === q.correctAnswer)
                    ...
                    const u = parseFloat(userAns), c = parseFloat(q.correctAnswer);
                    if (!isNaN(u) && !isNaN(c) && Math.abs(u - c) < 0.01)
                        conceptMap[concept].correct++;
                }
                else if (q.type === 'MSQ' && Array.isArray(userAns) && Array.isArray(q.correctAnswer) && JSON.stringify([...userAns].sort()) === JSON.stringify([...q.correctAnswer].sort()))
                    conceptMap[concept].correct++;
            });
        });
    ...
    return (<div className="animate-in">
      ...
      <div className="px">
        ...
        {/* Concept Performance */}
        {conceptPerf.length > 0 && (<div style={{ marginBottom: '28px' }}>
            ...
            <div className="grid-2">
              {conceptPerf.map(c => (<div key={c.concept} className="card-static" style={{ padding: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
        ...
        {!campaigns?.length ? (<div className="card-static" style={{ padding: '32px', textAlign: 'center' }}>
            ...
            {campaigns.map(c => {
                ...
                return (<div key={c.id} className="campaign-card">
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => handleGenerateQR(c.id, c.title)} disabled={generating === c.id || !isActive} className="btn btn-primary btn-sm" style={{ justifyContent: 'center', opacity: (generating === c.id || !isActive) ? 0.5 : 1 }}>
                      {generating === c.id ? '...' : '+ New'}
                    </button>
