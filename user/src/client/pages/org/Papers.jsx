...
function generatePDFFromPaper(paper) {
  ...
  .ak-row{display:flex;gap:10px;padding:5px 0;border-bottom:1px solid #eee}.ak-num{font-weight:bold;width:40px}.ak-ans{flex:1;color:#16a34a;font-weight:bold}</style></head><body>`;
    ...
    questions.forEach((q, i) => {
        html += `<div class="q-block"><div class="q-title">Q${i + 1}. ${q.text} <span style="background:#e5e7eb;padding:2px 8px;border-radius:4px;font-size:11px;float:right">${q.type}</span></div>`;
        if (q.type === 'NAT')
...
export function OrgPapers() {
    ...
    filteredPapers.forEach(p => { const key = p.campaign?.title || 'Unknown'; if (!groupedByCampaign[key])
        groupedByCampaign[key] = []; groupedByCampaign[key].push(p); });
    ...
    return (<div className="animate-in">
      ...
      <div className="px">
        ...
        <div className="tabs" style={{ marginBottom: '20px' }}>
          {['all', 'attempted', 'not-attempted'].map(f => (<button key={f} onClick={() => setFilter(f)} className={`tab${filter === f ? ' active' : ''}`}>
        ...
        {!filteredPapers.length ? (<div className="card-static" style={{ padding: '40px', textAlign: 'center' }}>
          ...
          </div>) : (Object.entries(groupedByCampaign).map(([campaignTitle, campaignPapers]) => (<div key={campaignTitle} style={{ marginBottom: '24px' }}>
                ...
                return (<div key={paper.id} className="card card-glow" style={{ overflow: 'hidden', padding: 0 }}>
                      <div style={{ padding: '14px 20px', cursor: 'pointer' }} onClick={() => setExpanded(isExpanded ? null : paper.id)}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                          <div>
                            {paper.orgStudent && (<div style={{ fontWeight: 600, fontSize: '14px', marginBottom: '4px', color: 'var(--blue)' }}>
