...
export function OrgSettings() {
    ...
    useEffect(() => {
        if (config)
            setWeights({ weightAssignment: config.weightAssignment, weightBehavior: config.weightBehavior, weightPerformance: config.weightPerformance, weightClassTest: config.weightClassTest });
    }, [config]);
    if (!weights)
    ...
    const handleSave = async () => {
        ...
        setError('');
        try {
            await updateConfig({ orgId: org.id, ...weights });
            setSaved(true);
            refetch();
    ...
    const WeightRow = ({ label, field }) => (<div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', background: 'var(--bg0)', borderRadius: '8px', border: '1px solid var(--border2)' }}>
      <span style={{ flex: 1, fontSize: '14px' }}>{label}</span>
      <input type="number" value={Math.round(weights[field] * 100)} onChange={e => setWeights({ ...weights, [field]: (parseFloat(e.target.value) || 0) / 100 })} className="form-input" style={{ width: '72px', textAlign: 'center', padding: '6px' }} min="0" max="100" step="5"/>
    ...
    return (<div className="animate-in">
      ...
      <div className="px" style={{ maxWidth: '560px' }}>
        ...
        <div className="card card-glow">
          ...
          <button onClick={handleSave} disabled={Math.abs(total - 1.0) > 0.01} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '12px', opacity: Math.abs(total - 1.0) > 0.01 ? 0.5 : 1 }}>
            Save Weights
          </button>
        </div>
      </div>
