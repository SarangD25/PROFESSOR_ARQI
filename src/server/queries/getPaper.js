  ...
  try {
    ...
    if (vCleaned.startsWith('{')) {
      try {
        ...
        for (const key of Object.keys(obj)) {
          if (Array.isArray(obj[key])) { vCleaned = JSON.stringify(obj[key]); break; }
        }
      } catch (_) { }
    }
    ...
    const result = questions.map((q, i) => {
      ...
      if (q.type === 'MCQ') {
        ...
        if (origNorm === verifyNorm) { kept++; return q; }
        ...
        const partialMatch = q.options.find(o =>
          o.toLowerCase().includes(verifyNorm) || verifyNorm.includes(o.toLowerCase().trim())
        );
        if (partialMatch) {
        ...
        if (JSON.stringify(origArr) === JSON.stringify(verifyArr)) { kept++; return q; }
...
{"text": "${exampleQuestion.text}", "type": "${exampleQuestion.type}", "options": ${JSON.stringify(exampleQuestion.options)}, "correctAnswer": ${JSON.stringify(exampleQuestion.correctAnswer)}, "marks": ${exampleQuestion.marks}, "negativeMarks": ${exampleQuestion.negativeMarks}, "concept": "${exampleQuestion.concept}"}`;
  ...
  if (isDBAMS) {
    ...
    const seen = new Set();
    conceptSlots = all.filter(c => { if (seen.has(c)) return false; seen.add(c); return true; }).slice(0, 15);
  }
  ...
  const schemaHints = pickedSchemas.map((s, i) =>
    `Normalization scenario ${i + 1}: R(${s.attrs}) with FDs: ${s.fds}`
  ).join('\n');

  // Build SQL hints
  const sqlHints = pickedSqlCtx.map((s, i) =>
    `SQL scenario ${i + 1}: table ${s.table}(${s.cols})`
  ).join('\n');
...
export async function getPaper(args, context) {
  ...
  if (!questionSet) {
    ...
    const previousQuestions = previousPapers
      ...
      .flatMap(qs => {
        try { return JSON.parse(qs.questions); } catch (e) { return []; }
      });
...
function generateFallbackQuestions(topic, examName, difficulty) {
  ...
  if (!pool) {
    ...
    else if (['english', 'synonym', 'antonym', 'grammar', 'vocabulary', 'comprehension'].some(k => topicLower.includes(k))) pool = fallbackPools['Chemistry']; // safe general fallback
