...
function TrendChart({ points, width = 600, height = 200 }) {
  if (points.length < 2) return <p style={{ color: 'var(--text3)', fontSize: '13px' }}>Need at least 2 tests for trend chart</p>;
  const padding = 40; const w = width - padding*2; const h = height - padding*2;
...
export function IndividualAnalytics() {
  ...
  const analytics = useMemo(() => {
    ...
    const topicMap = {};
    attempted.forEach(p => { const topic = p.campaign?.topic || 'Unknown'; const att = p.questionSets[0].attempts[0]; const pct = Math.round((att.score/att.totalMarks)*100);
      if (!topicMap[topic]) topicMap[topic] = { total: 0, count: 0 }; topicMap[topic].total += pct; topicMap[topic].count++; });
    ...
    attempted.forEach(p => { const qs = p.questionSets[0]; let questions = [], answers = [];
      try { questions = typeof qs.questions === 'string' ? JSON.parse(qs.questions) : qs.questions; } catch { questions = []; }
      try { answers = typeof qs.attempts[0].answers === 'string' ? JSON.parse(qs.attempts[0].answers) : qs.attempts[0].answers; } catch { answers = []; }
      questions.forEach((q,idx) => { const t = q.type||'MCQ'; if (typeStats[t]) { typeStats[t].total++; if (checkClient(answers[idx], q.correctAnswer, t)) typeStats[t].correct++; } });
    });
    ...
    attempted.forEach(p => { const qs = p.questionSets[0]; let questions = [], answers = [];
      try { questions = typeof qs.questions === 'string' ? JSON.parse(qs.questions) : qs.questions; } catch { questions = []; }
      try { answers = typeof qs.attempts[0].answers === 'string' ? JSON.parse(qs.attempts[0].answers) : qs.attempts[0].answers; } catch { answers = []; }
      questions.forEach((q,idx) => { const c = q.concept||p.campaign?.topic||'General'; if (!conceptMap[c]) conceptMap[c] = { correct: 0, total: 0 }; conceptMap[c].total++; if (checkClient(answers[idx], q.correctAnswer, q.type)) conceptMap[c].correct++; });
    });
    ...
    if (avgScore >= 80) { recommendation = 'Excellent performance! Try harder difficulty to push your limits.'; recLevel = 'hard'; }
    else if (avgScore >= 60) { recommendation = 'Good progress! Focus on weak areas and try medium-hard questions.'; recLevel = 'medium-hard'; }
    else if (avgScore >= 40) { recommendation = 'Keep practicing! Review fundamentals, then attempt medium difficulty.'; recLevel = 'medium'; }
    else { recommendation = 'Start with basics. Review theory before attempting questions. Try easy difficulty.'; recLevel = 'easy'; }
  ...
  if (isLoading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', color: 'var(--text3)' }}>Loading analytics...</div>;
            ...
            <div className="grid-2" style={{ marginBottom: '16px' }}>
              <div className="tier-card tier-low" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '16px' }}>
                ...
                {analytics.weakAreas.length === 0 ? <p style={{ fontSize: '13px', color: 'var(--text3)' }}>No weak areas — great job!</p> :
                  analytics.weakAreas.map(c => (
                    <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '13px' }}>
