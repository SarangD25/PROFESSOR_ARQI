...
function generatePDFFromPaper(paper) {
  ...
  if (attempt) { const pct = Math.round((attempt.score / attempt.totalMarks) * 100); html += `<div class="score-box"><div class="big">${attempt.score}/${attempt.totalMarks}</div><div>${pct}%</div></div>`; }
  questions.forEach((q, idx) => {
    html += `<div class="q-block"><div class="q-title"><span class="q-type">${q.type}</span>Q${idx+1}. ${q.text}</div>`;
    if (q.type === 'NAT') { html += `<div style="margin-left:20px;color:#555">Answer: ${q.correctAnswer}</div>`; }
    else { (q.options||[]).forEach((opt,i) => { html += `<div class="opt">${String.fromCharCode(65+i)}. ${opt}</div>`; }); }
    html += `</div>`;
  });
...
export function IndividualPapers() {
  ...
  const handleDelete = async (paperId) => {
    if (confirm('Delete this paper? This cannot be undone.')) { await deleteAction({ paperId }); refetch(); }
  };
                  ...
                  {isExpanded && (
                    <div style={{ borderTop: '1px solid var(--border)', padding: '16px 20px' }}>
                      ...
                      {questions.map((q, idx) => {
                        ...
                        return (
                          <div key={idx} className="card-static" style={{ marginBottom: '8px', borderLeft: `3px solid ${attempt ? (isCorrect ? 'var(--green)' : 'var(--red)') : 'var(--border2)'}`, padding: '12px' }}>
                            ...
                            {q.type !== 'NAT' && q.options?.map((opt, i) => {
                              ...
                              return (
                                <div key={i} style={{ fontSize: '12px', padding: '4px 0 4px 16px', color: isCor ? 'var(--green)' : 'var(--text3)' }}>
                            ...
                            {attempt && (
                              <div style={{ fontSize: '12px', paddingLeft: '16px', marginTop: '4px' }}>
