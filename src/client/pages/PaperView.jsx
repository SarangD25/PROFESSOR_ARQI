...
function checkAnswerClient(userAns, correctAns, type) {
  ...
  if (type === 'MCQ') return userAns === correctAns;
  if (type === 'NAT') {
...
export function PaperView() {
  ...
  if (isLoading) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 'calc(100vh - 56px)', color: 'var(--text3)' }}>
  ...
  if (result) {
    ...
    return (
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px 32px' }} className="animate-in">
        {/* Score Card */}
        ...
        {result.weakTopics && result.weakTopics.length > 0 && (
          <div className="tier-card tier-low" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '20px', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '15px', fontWeight: 700, color: 'var(--red)', marginBottom: '12px' }}>Topics to Improve</h3>
            {result.weakTopics.map((wt, i) => (
              <div key={i} className="card-static" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', marginBottom: '6px' }}>
        ...
        {result.conceptResults && (
          <div className="tier-card tier-excellent" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '20px', marginBottom: '24px' }}>
        ...
        {paper?.questions?.map((q, idx) => {
          ...
          return (
            <div key={idx} className="card-static" style={{ marginBottom: '10px', borderLeft: `3px solid ${isCorrect ? 'var(--green)' : 'var(--red)'}` }}>
              ...
              {q.type === 'NAT' ? (
                ...
                <div style={{ marginTop: '8px' }}>
                  {q.options?.map((opt, i) => {
                    ...
                    let bg = 'transparent';
                    if (isCorrectOpt) { borderColor = 'var(--green)'; bg = '#22c55e12'; }
                    else if (isUserPick) { borderColor = 'var(--red)'; bg = '#ef444412'; }
                    return (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', border: `1px solid ${borderColor}`, borderRadius: '10px', marginBottom: '6px', background: bg, fontSize: '13.5px' }}>
  ...
  return (
    <div className="test-wrap">
      ...
      <div className="test-main">
        ...
        {/* Questions */}
        {paper?.questions?.map((q, idx) => (
          <div key={idx} className="card-static" style={{ marginBottom: '14px', borderLeft: answers[idx] !== undefined && answers[idx] !== null && answers[idx] !== '' ? '3px solid var(--green)' : '3px solid var(--border2)' }}>
            ...
            {q.type === 'MSQ' && (
              <p style={{ fontSize: '11px', color: 'var(--amber)', marginBottom: '8px' }}>Select ALL correct answers (multiple may be correct)</p>
            )}
            ...
            {q.type === 'NAT' ? (
              <div>
                ...
                <input type="number" step="0.001" placeholder="e.g., 3.142" value={answers[idx] || ''}
            ...
            /* MSQ */
              ...
              <div>
                {q.options?.map((opt, i) => {
                  ...
                  return (
                    <div key={i} className={`mcq-option${selected ? ' selected' : ''}`}
                      onClick={() => {
                        const current = Array.isArray(answers[idx]) ? [...answers[idx]] : [];
                        if (selected) setAnswers({ ...answers, [idx]: current.filter(a => a !== opt) });
                        else setAnswers({ ...answers, [idx]: [...current, opt] });
                      }}>
            ...
            /* MCQ */
              ...
              <div>
                {q.options?.map((opt, i) => (
                  <div key={i} className={`mcq-option${answers[idx] === opt ? ' selected' : ''}`}
        ...
        <button onClick={handleSubmit} disabled={submitted} className="btn btn-primary"
          style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: '15px', marginTop: '8px', marginBottom: '32px', opacity: submitted ? 0.5 : 1 }}>
      ...
      <div className="test-sidebar">
        ...
        <div className="q-palette">
          {paper?.questions?.map((_, idx) => {
            ...
            return (
              <div key={idx} className={`q-num${isAnswered ? ' answered' : ''}${currentQ === idx ? ' current' : ''}`}
                onClick={() => { setCurrentQ(idx); document.querySelectorAll('.card-static')[idx + 1]?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }}>
                {idx + 1}
              </div>
            );
          })}
