...
export function OrgAddAssessment() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try { await create({ orgId: org.id, entries }); navigate('/org/assessments'); }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="card card-glow" style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {students?.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'var(--bg0)', borderRadius: '8px', border: '1px solid var(--border2)' }}>
                  ...
                  <input type="number" placeholder="—" value={marks[s.id] || ''}
                    onChange={e => setMarks({...marks, [s.id]: e.target.value})}
                    className="form-input" style={{ width: '80px', textAlign: 'center', padding: '6px' }} min="0" max={form.maxMarks} step="0.5" />
